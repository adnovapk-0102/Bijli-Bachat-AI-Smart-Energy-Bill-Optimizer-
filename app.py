...
bill_status, bill_text = bill_analysis_status(monthly_bill, previous_bill, "bill")
units_status, units_text = bill_analysis_status(monthly_units, previous_units, "units")

if monthly_units > 0 and total_appliance_kwh > monthly_units * 1.05:
    appliance_status = "alert"
    appliance_text = "Entered appliance use exceeds billed units"
elif monthly_units > 0 and total_appliance_kwh > monthly_units * 0.75:
    appliance_status = "warning"
    appliance_text = "High share of billed consumption"
else:
    appliance_status = "good"
    appliance_text = "Within the billed consumption range"

if bill_status == "alert" or units_status == "alert":
    overall_status = "alert"
    overall_text = "High-priority energy review recommended"
elif bill_status == "good" and units_status in ("good", "neutral"):
    overall_status = "good"
    overall_text = "Your energy trend is improving"
else:
    overall_status = "warning"
    overall_text = "Monitor the next billing cycle"
...
st.markdown(
    ...
    <div class="snapshot-grid">
        ...
        {status_card("Top Appliance Load", f"{total_appliance_kwh:,.0f} kWh", appliance_status, appliance_text)}
        {status_card("Overall Status", "Review" if overall_status == "alert" else ("Good" if overall_status == "good" else "Monitor"), overall_status, overall_text)}
    </div>
    """,
    unsafe_allow_html=True,
)

# Feature 1 — Electricity Bill Analyzer
st.markdown('<div class="section-title">⚡ Electricity Bill Analyzer</div>', unsafe_allow_html=True)

if previous_bill > 0:
    bill_delta = monthly_bill - previous_bill
    bill_pct = (bill_delta / previous_bill) * 100
else:
    bill_delta, bill_pct = None, None

if previous_units > 0:
    units_delta = monthly_units - previous_units
    units_pct = (units_delta / previous_units) * 100
else:
    units_delta, units_pct = None, None

analysis_cols = st.columns(4)
analysis_cols[0].metric(
    "Current Bill", money(monthly_bill),
    f"{bill_pct:+.1f}%" if bill_pct is not None else "No previous bill"
)
analysis_cols[1].metric(
    "Current Units", f"{monthly_units:,.0f} kWh",
    f"{units_pct:+.1f}%" if units_pct is not None else "No previous units"
)
analysis_cols[2].metric(
    "Appliance Coverage",
    f"{appliance_share:.0f}%",
    "of billed units explained"
)
...
if bill_pct is not None and bill_pct >= 10 and (units_pct is None or units_pct <= 0):
    st.markdown(
        f'<div class="warning-card">⚠️ <b>Bill anomaly:</b> the bill increased by <b>{bill_pct:.1f}%</b> while units did not increase. Tariff slabs, taxes, fixed charges or other bill components may contribute. Review the bill details before assuming appliance overuse.</div>',
        unsafe_allow_html=True,
    )
elif bill_pct is not None and bill_pct >= 10:
    st.markdown(
        f'<div class="warning-card">🚨 <b>High bill alert:</b> your bill increased by <b>{bill_pct:.1f}%</b>. The appliance audit below shows where to investigate first.</div>',
        unsafe_allow_html=True,
    )
elif bill_pct is not None and bill_pct <= -5:
    st.markdown(
        f'<div class="success-card">✅ <b>Good news:</b> your bill is down <b>{abs(bill_pct):.1f}%</b> compared with the previous month.</div>',
        unsafe_allow_html=True,
    )

if monthly_units > 0 and total_appliance_kwh > monthly_units * 1.05:
    st.markdown(
        '<div class="warning-card">⚠️ <b>Consumption gap:</b> the appliance schedule you entered estimates more usage than the billed units. Review appliance hours, power ratings and missing/duplicate entries.</div>',
        unsafe_allow_html=True,
    )
...
# -----------------------------
# Feature 5 — Better Monitoring
# -----------------------------
st.markdown("---")
st.markdown('<div class="section-title">📈 Energy Monitoring</div>', unsafe_allow_html=True)

monitor_items = []
if bill_pct is not None:
    monitor_items.append(("Bill trend", bill_pct))
if units_pct is not None:
    monitor_items.append(("Units trend", units_pct))

if monitor_items:
    monitor_df = pd.DataFrame(monitor_items, columns=["Metric", "Change %"]).set_index("Metric")
    st.bar_chart(monitor_df)
else:
    st.info("Enter previous-month bill and unit data to unlock trend monitoring.")

if bill_pct is not None and units_pct is not None:
    if bill_pct > units_pct + 10:
        st.markdown(
            '<div class="warning-card">🔎 <b>Tariff/charges signal:</b> your bill is rising materially faster than your unit consumption. Review tariff slabs, taxes and fixed charges.</div>',
            unsafe_allow_html=True,
        )
    elif units_pct >= 10:
        st.markdown(
            '<div class="warning-card">🔎 <b>Usage signal:</b> your unit consumption is rising materially. Review the highest-consuming appliances first.</div>',
            unsafe_allow_html=True,
        )
    elif bill_pct <= -5 and units_pct <= -5:
        st.markdown(
            '<div class="success-card">✅ <b>Positive trend:</b> both bill and unit consumption are moving down.</div>',
            unsafe_allow_html=True,
        )
...
st.markdown(
    '<div class="small-muted">Judge view: bill analysis → appliance diagnosis → personalized action plan → next-bill estimate → monitoring. All calculations are transparent estimates based on user-entered data.</div>',
    unsafe_allow_html=True,
)
