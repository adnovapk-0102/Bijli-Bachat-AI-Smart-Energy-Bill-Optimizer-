...
<style>
    ...
    /* High-contrast, professional Streamlit tabs */
    ...
    button[data-baseweb="tab"][aria-selected="true"] {
        color: #ffffff !important;
        background: linear-gradient(135deg, #0f172a, #166534) !important;
...
for tab, role in zip(tabs, roles):
    with tab:
        report_name = role.replace(" Agent", "")
        if st.button(
            f"Generate {report_name} Analysis",
            key=f"run_{role}",
            use_container_width=True,
        ):
