...
st.markdown("""
<style>
.stApp { background: linear-gradient(135deg, #f7fbff 0%, #eef7f1 100%); color: #0f172a !important; }
.main .block-container { color: #0f172a !important; max-width: 1250px; padding-top: 2rem; }
.main .block-container p, .main .block-container span, .main .block-container label, .main .block-container div, .main .block-container li, .main .block-container h1, .main .block-container h2, .main .block-container h3, .main .block-container h4 { color: inherit; }
[data-testid="stSidebar"] { background: #0f172a; }
[data-testid="stSidebar"] * { color: #f8fafc !important; }
[data-testid="stSidebar"] input, [data-testid="stSidebar"] textarea, [data-testid="stSidebar"] [data-baseweb="select"] > div { background: #ffffff !important; color: #0f172a !important; }
[data-testid="stSidebar"] [data-baseweb="select"] * { color: #0f172a !important; }
.hero { padding: 2rem 2.2rem; border-radius: 24px; background: linear-gradient(135deg, #0f172a 0%, #164e63 55%, #166534 100%); color: white !important; margin-bottom: 1.2rem; box-shadow: 0 12px 35px rgba(15, 23, 42, .18); }
...
.card { background: #ffffff; border: 1px solid #e2e8f0; border-radius: 18px; padding: 1.1rem; box-shadow: 0 6px 20px rgba(15, 23, 42, .06); margin-bottom: .8rem; color: #0f172a !important; }
.card * { color: #0f172a !important; }
.metric-title { color: #64748b !important; font-size: .86rem; }
.metric-value { color: #0f172a !important; font-size: 1.55rem; font-weight: 750; }
.badge { display: inline-block; padding: .28rem .7rem; border-radius: 999px; font-size: .78rem; font-weight: 700; background: #dcfce7; color: #166534 !important; }
.section-title { color: #0f172a !important; font-size: 1.35rem; font-weight: 750; margin-top: .6rem; margin-bottom: .5rem; }
.small-muted { color: #64748b !important; font-size: .88rem; }
.warning-card { border-left: 5px solid #f59e0b; background: #fffbeb; color: #78350f !important; padding: .9rem 1rem; border-radius: 12px; }
.warning-card * { color: #78350f !important; }
.success-card { border-left: 5px solid #16a34a; background: #f0fdf4; color: #166534 !important; padding: .9rem 1rem; border-radius: 12px; }
.success-card * { color: #166534 !important; }
.stTabs [data-baseweb="tab-list"] { gap: .4rem; background: #ffffff; padding: .35rem; border-radius: 14px; border: 1px solid #e2e8f0; }
.stTabs [data-baseweb="tab"] { color: #0f172a !important; background: #ffffff !important; border-radius: 10px; font-weight: 700; }
.stTabs [aria-selected="true"] { color: #0f172a !important; background: #e2e8f0 !important; }
[data-testid="stMetric"] { background: #ffffff; border: 1px solid #e2e8f0; border-radius: 16px; padding: .8rem; box-shadow: 0 5px 18px rgba(15, 23, 42, .05); }
[data-testid="stMetricLabel"] *, [data-testid="stMetricValue"] * { color: #0f172a !important; }
[data-testid="stDataFrame"] { background: #ffffff !important; border-radius: 14px; }
...
st.caption("A quick overview of the electricity data you entered. Values marked as estimated are calculated from your appliance wattage, daily usage and approximate tariff.")
...
with left:
    ...
    if not df.empty:
        ...
        display_df = display_df.rename(columns={
            "Power (W)": "Power",
            "Hours/day": "Hours / Day",
            "Monthly kWh": "Monthly Units (kWh)",
            "Estimated Cost": "Est. Cost (PKR)",
        })
        st.dataframe(display_df, use_container_width=True, hide_index=True)
        st.caption("Estimated Cost = estimated appliance kWh × the approximate per-unit rate you entered. Your real electricity bill can differ because Pakistani bills may include slab rates, taxes, fixed charges and other adjustments.")
...
st.info(
    f"Estimated footprint: **{estimated_co2:,.1f} kg CO₂e/month** using an illustrative "
    f"grid factor of {GRID_CO2_FACTOR:.2f} kg CO₂e/kWh. This is a planning estimate, "
    "not an official Pakistan grid-emissions factor."
)
...
# -----------------------------
st.markdown("---")
st.markdown('<div class="section-title">🤖 AI Energy Audit</div>', unsafe_allow_html=True)
    ...
    with tab:
        if st.button(
            ...
            if result.startswith("__AI_ERROR__"):
                st.error("AI request failed. Please check the Gemini API key, model availability, and Streamlit Secrets.")
                with st.expander("Technical details"):
                    st.code(result.replace("__AI_ERROR__", ""))
            else:
                st.markdown(result)
...
# -----------------------------
st.markdown("---")
st.markdown('<div class="section-title">💬 Ask My Energy AI</div>', unsafe_allow_html=True)
question = st.chat_input(
    "Example: Why is my bill high? How can I reduce AC usage?"
)
...
if question:
    ...
    with st.chat_message("assistant"):
        with st.spinner("Analyzing your household data..."):
            answer = run_ai_analysis(
                user_data,
                "General Energy Assistant",
                question,
            )
        if answer.startswith("__AI_ERROR__"):
            st.error("AI request failed. Please check your Gemini API key and Streamlit Secrets.")
            with st.expander("Technical details"):
                st.code(answer.replace("__AI_ERROR__", ""))
        else:
            st.write(answer)
...
st.markdown("---")
st.caption(
    "Bijli Bachat AI • Environment & Sustainability — Tech for Good • "
    "Hackathon MVP • Calculations are estimates based on user-provided data."
)
